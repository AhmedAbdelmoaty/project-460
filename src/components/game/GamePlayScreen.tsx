...
interface GamePlayScreenProps {
  ...
  onRejectHypothesis: (hypothesisId: HypothesisId, evidenceIds: EvidenceId[]) => { success: boolean; message: string };
...
export function GamePlayScreen({
  ...
  const progressPercent = (stepsUsed / 4) * 100;
  
  // Actions grid: عرض أفعال التحقيق فقط
  const mainActions = mainScenario.actions.filter(
    (a) => a.id !== 'declare_solution' && a.id !== 'end_investigation'
  );
        ...
        <div className="flex flex-wrap gap-4 items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <Badge variant={remainingSteps <= 1 ? "destructive" : "secondary"} className="text-sm px-3 py-1">
              👣 خطوات التحقيق: {remainingSteps}/4
            </Badge>
          </div>
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            <AlertDialog>
              ...
              <AlertDialogContent dir="rtl">
                <AlertDialogHeader>
                  <AlertDialogTitle>إنهاء التحقيق بدون قرار؟</AlertDialogTitle>
                  <AlertDialogDescription>
                    لو عملت كده، المحاولة دي هتتحسب "بدون قرار".
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  <AlertDialogCancel>تراجع</AlertDialogCancel>
                  <AlertDialogAction onClick={onEndInvestigation}>
                    إنهاء
                  </AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
